...
속성 중 하나는 `SQLResultSet` SQL 쿼리에서 반환 된 행을 포함합니다.
...
`SQLResultSetRowList`SQL에서 반환 된 데이터를 포함 `select` 문. 개체를 포함 한 `length` 속성 행을 나타내는 `select` 문 반환 합니다. 데이터의 행을 가져오려면 호출에 `item` 인덱스를 지정하는 방법. 자바스크립트를 반환 합니다 `Object` 속성은 데이터베이스 열에 `select` 문이 실행 되었습니다.
...
## SQL 빠른 예제 실행
    ...
    querySuccess (tx, 결과) 함수 {var len = results.rows.length;
            ...
            에 대한 (var i = 0; 나 < 렌; i + +) {console.log ("행 =" + i + "ID =" + results.rows.item (i).id + "데이터 =" + results.rows.item(i).data);
